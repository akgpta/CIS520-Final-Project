...
\section{Abstract}
Suicide is a major public health concern worldwide. Because living in a different countries and therefore different environment can greatly impact one's lifestyle, we wanted to explore suicide rates over various countries. Specifically, we considered $80$ countries. For each country, we collected the following data: \TODO. We were interested in understanding how these factors could help predict suicide rates. To do this, we implemented $k$-Nearest Neighbors, Linear Regression, and Neural Networks. brief results: \TODO
...
\subsection{Performance Measures} Given the regression nature of this problem, we used squared loss model to measure our performance. This is because with positive real valued data with explicit bounds of $[0, 10^5]$, but more reasonable empirical bounds of around $[0, 10^3]$. The following image is a representation of squared loss,but in a one dimensional feature space \endnote{https://blog.algorithmia.com/introduction-to-loss-functions/\textbf{}}. 
\includegraphics[width=\columnwidth]{squared-loss.png}


\section{Solution Methods and Algorithms}
\subsection{Algorithms Used} We implemented a variety of models to help predict suicide rates by country. \TODO (idk if need general overview - but probably delete later)

\subsection{$k$ Nearest-Neighbors} 

- good stepping stone

- thought first would be good to compare countries to other countries

- 

We first started with nearest neighbors and found through cross validation that 1-nearest neighbors worked the best. We also tried using Minkowski’s distance and similarly found that 1-nearest neighbors using Euclidean distance produced the best model. 
...
\subsection{Neural Network} When we ran neural net with 1 hidden layer, we saw that our model only got better as the model’s complexity increased, further showing that our model was essentially grossly overfitting to the train data and essentially functioning as a memorizer. 
\subsection{Linear Regression} As most related papers ran linear regression, we also ran regularized and unregularized least squares linear regression to see if perhaps we could prevent this overfitting. Ehhhhh

\section{Experimental Design and Results}
\subsection{Experiments} \TODO Describe how you set up your experiments, including choice
