...
The graph below shows the results when cross-validating over different $k$ values. We have omitted the training and test error to highlight an interesting trend in cross-validation error. Recall that each of our folds have data from each country of $3$ years. Say we are looking for the nearest neighbor(s) for country $x$. We suspect that the error is very low for $k$ values of $1, 2, 3$ because the model is utilizing the country $x$'s $3$ data points from different years to predict suicide rates for the test year. Errors pick up for values of $k$ greater than $3$ because the model is forced to use the data from countries other than country $x$, which may not be as indicative when making decision about suicide rates for country $x$. 
...
When we ran a neural net with 1 hidden layer, we saw that our model only got better as the model’s complexity increased, further showing that our model was grossly overfitting to the train data and functioning as a memorizer.

\includegraphics[width=\columnwidth]{neur-net.png}

\subsection{Linear Regression} First, we implemented linear regression as our baseline model. 


As most related papers ran linear regression, we also ran regularized and unregularized least squares linear regression to see if we could prevent this overfitting. This is our base line model \TODO(maybe say excessive details are left out for the sake of parsimony?) \TODO 

\includegraphics[width=\columnwidth]{lin_reg.png}

\subsection{Overall Results}

\TODO not a complete sentence Ended up with memorizers on neural net and nearest neighbors.
