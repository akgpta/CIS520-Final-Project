...
\subsection{$k$ Nearest-Neighbors} We first implemented a $k$-nearest neighbor algorithm using Euclidean distance as we suspected that suicide rates between similar countries would be comparable. 

- good stepping stone,  thought first would be good to compare countries to other countries

- With Euclidean distance, we found $k=1$ was the best

- Also tried Minkowski's distance, a more general distance formula

\[D\left(\mathbf{x}_{i}, \mathbf{x}_{j}\right)=\left(\sum_{l=1}^{d}\left|x_{i l}-x_{j l}\right|^{1 / p}\right)^{p}\]

- similarly found k = 1, p = 2 (eucledian distance) produced best model
