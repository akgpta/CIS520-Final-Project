...
\subsection{$k$ Nearest-Neighbors} We first implemented a $k$-nearest neighbor algorithm using Euclidean distance as we suspected that suicide rates between similar countries would be comparable. We cross validated across the following values of $k = [1, 2, 5, 10]$ and found that $k = 1$ produced the best model. 
...
We cross validated across the following values of $p = [1, 2, 3]$ and for each of these $p$ values, found the best $k = 1$ value.
